...
const Materials = () => {
	const {setIsAnimating, isAnimating, setCountMaterials, countMaterials, setHovered, hovered} = useMaterials()
	const handleShowAll = () => {
		setIsAnimating(true);
	...
	return (
		<section className="py-[80px] bg-white materials" id='materials'>
			<div className='container flex flex-col items-center'>
				<h2 className="text-[28px] mb-[13px] uppercase font-extrabold">Полезные материалы</h2>
				<Line/>
				<p className="mt-[24px] mb-[48px] text-center max-w-[920px] font-normal">
					Актуальные новости и экспертные материалы из мира права: всё, что нужно знать для защиты ваших интересов
				</p>
				<motion.ul className="flex items-center   mb-10 gap-4 flex-wrap"
									 initial={{opacity: 0}}
									 animate={{opacity: 1}}
									...
									{hovered === index && (
										...
										>
											...
											<div className="p-[15px] bg-white rounded-lg shadow-lg">
												<img src="/img/Materials/show.png" alt="img" className='object-cover'/>
											</div>
										</motion.div>
									)}
								</div>
								<p
									className="font-bold mb-[16px] group-hover:text-[#4F8FF0] duration-[0.5s] materials__title">{material.title}</p>
								<p className="mb-[16px] font-light text-[12px]">{material.date}</p>
