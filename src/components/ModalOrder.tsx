...
const ModalOrder = () => {
	...
	const validationSchema = Yup.object().shape({
		name: Yup.string().required('Имя обязательно')
			.matches(/^[A-Za-zА-Яа-яЁё\s-]+$/, 'Имя должно содержать только буквы и пробелы')
	...
	return (
		<AnimatePresence>
			<motion.div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-10'
			...
			>
				<div
					className='h-[520px] w-[600px] bg-white flex flex-col items-center justify-center p-8 rounded-lg shadow-xl transition-transform transform  relative'>
					<RxCross2
						...
						onClick={closeModal}
					/>
					<h2 className="text-2xl font-bold mb-6 text-gray-800 mt-5 text-left">Оставьте нам заявку и <br /> мы с вами
						свяжемся!</h2>
					<form className='w-full flex flex-col items-center justify-center' onSubmit={formik.handleSubmit}>
						<div className='mb-5'>
							...
							{formik.errors.phone && <div className="text-red-500 text-xs">{formik.errors.phone}</div>}
							{formik.errors.message && <div className="text-red-500 text-xs">{formik.errors.message}</div>}
						</div>
						<input
							minLength={2}
							maxLength={30}
							...
							style={formik.errors.name ? { borderColor: 'red' } : { borderColor: 'gray' }}
						...
						<input
							...
							style={formik.errors.phone ? { borderColor: 'red' } : { borderColor: 'gray' }}
						...
						<textarea
							...
							style={formik.errors.message ? { borderColor: 'red' } : { borderColor: 'gray' }}
